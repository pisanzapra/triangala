...
int TRAIN_SEARCH_DEPTH = 2; // egitimin OLGUN kismindaki arama derinligi
int PLAY_SEARCH_DEPTH = 4;  // insana karsi oynarken kullanilan arama derinligi (tek hamle icin daha fazla sure ayirabiliriz)
...
void runTraining(long long episodes) {
    ...
    for (long long ep = 1; ep <= episodes; ++ep) {
        ...
        // ISINMA (curriculum): V henuz gurultuluyken derin arama hatalari buyutur
        // (kotu bir degerlendirmeyi "sonuna kadar arar"). Once V'yi 1-adima esdeger
        // sig aramayla (depth=1) makul bir seviyeye getir, sonra derinlestir.
        int searchDepth = (frac < 0.25) ? 1 : TRAIN_SEARCH_DEPTH;
        ...
        if (ep % checkpoint == 0) {
            ...
            cout << "Episode " << ep << "/" << episodes << "  (sure: " << sec << "s)\n";
